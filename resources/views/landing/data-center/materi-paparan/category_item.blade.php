@forelse ($categoryDataCenter as $category)
    <div class="mb-3 col-3">
            <a href="{{ route('materi_paparan.category', [$slug, $category->slug]) }}"
        class="gap-2 p-2 document-list-card text-decoration-none d-flex align-items-center">
            <i class="bi bi-journal-bookmark bappeda-primary-color fs-3"></i>
            <p class="p-0 m-0 bappeda-primary-color fs-7 lh-sm">{{ translate($category->name) }}</p>
        </a>
    </div>
@empty
