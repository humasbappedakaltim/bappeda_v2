...
class SecureHeadersMiddleware
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        $response->headers->set('Strict-Transport-Security', 'max-age=31536000; includeSubDomains');
            ...
            //   $response->headers->set('Content-Security-Policy', "default-src 'self'; style-src 'self'; script-src 'self'; img-src 'self'; frame-src 'self'; font-src 'self';");
              $response->headers->set('Content-Security-Policy', "default-src 'self'; style-src 'self'; script-src 'self' 'unsafe-eval'; img-src 'self'; frame-src 'self'; font-src 'self';");
